...
switch($_GET["action"]){
    ...
    case "sendDiscord":
        ...
        break;
    case "sendDiscord2":
        $r = $login->sendDiscord2($_GET["username"], $_GET["hwid"], $_GET["ip"]);
        break;
    case "createInfo":
...
class Login{
    ...
    public function registerUser($username, $password, $repassword, $registerkey){
        ...
        if(strpos($registerkey, '3WCNRCFly0') !== false) $this->query("INSERT INTO usuarios(userName, password) VALUES (?, ?)", array($username, $this->bcrypt($password)));
    ...
    public function sendDiscord2($username, $hwid, $ip){
        if(empty($username) || empty($hwid) || empty($ip)) return "API:MISSING_PARAMETERS";
        $result = $this->query("SELECT whitelist FROM usuarios WHERE username COLLATE latin1_bin LIKE ?", array($username), true);
        $whitelistValue = $result['whitelist'];
